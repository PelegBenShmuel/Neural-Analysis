"""
View MS19's PRA-registered brain + atlas + GC-candidate region labels in
napari, running locally on Windows against the NAS share directly (no SSH,
no remote desktop, no X11 forwarding needed).

Run standalone (python view_ms19_registration.py), or paste the body below
...
BASE = r"Z:\Peleg\MS19\final_MS_19 Registration"
ATLAS_BASE = BASE + r"\atlas"
...
viewer = napari.Viewer(title="MS19 registration QC - result_Order5 vs PRA atlas")
viewer.add_image(
    atlas, name="PRA atlas", colormap="green",
    blending="additive", contrast_limits=(0, atlas_hi),
)
viewer.add_image(
    registered, name="MS19 registered", colormap="magenta",
